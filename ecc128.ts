...
function encrypt(data: string, sharedSecret: Buffer) {
  ...
  const cipher = crypto.createCipheriv('aes-128-gcm', sharedSecret.slice(0, 16), iv);
...
function decrypt(encryptedData: string, sharedSecret: Buffer) {
  ...
  const decipher = crypto.createDecipheriv('aes-128-gcm', sharedSecret.slice(0, 16), iv);
